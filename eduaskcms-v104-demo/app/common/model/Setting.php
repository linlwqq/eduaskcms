...
class Setting extends App
{
    ...
    public function before_write()
    {
        ...
        if ($this->data['type'] == 'keyvalue' && is_array($this->data['value'])) {
            $this->data['value'] = Hash::combine($this->data['value'], '{n}.key', '{n}.value');
        }
        if (in_array($this->data['type'], $this->parseJsonTypes) && is_array($this->data['value'])) {
            $this->data['value'] = json_encode($this->data['value']);
        }
        return $rslt;
    }
    ...
    public function settingGroup(){
        return $this->belongsTo('SettingGroup');//->setEagerlyType(0);
    }
    */
