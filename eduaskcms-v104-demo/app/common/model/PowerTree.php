...
class PowerTree extends App
{
    ...
    {
        $this->data['controller'] = trim($this->data['controller']);
        $this->data['action'] = trim($this->data['action']);
        $this->data['together'] = strtolower($this->data['controller']) . '::' . strtolower($this->data['action']); 
    ...
    public function after_delete()
    {
        $parent_rslt = call_user_func(array('parent', __FUNCTION__));
        if ($this->data['id']) {                   
            $this->where(array('id' => array('IN', powertree('children', $this->data['id']))))->delete();##删除掉所有子级
        }        
        $this->writeToFile();
        return $parent_rslt;
    }
