...
class Run extends App
{
    ...
    public function create()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if(isset($this->args['copy_id'])) {
                ...
                $copy_data = $this->mdl->where(array_merge((array)$this->local['where'], array($pk => array('eq', $copy_id))))->find();
        ...
        $this->setTitle("新增{$this->mdl->cname}", 'operation');
    ...
    public function ajax_set_field()
    {
        ...
        $pk = $this->mdl->getPk();
        $old_data = $this->mdl->where(array_merge((array)$this->local['where'], array($pk => array('eq', $id))))->find();
        if (empty($old_data)) {
    ...
    public function modify()
    {
        ...
        if ($this->request->isPost() && $old_data) {
            ...
            $this->mdl->data($this->Form->data[$this->m]);
            ##执行更新            
            $result = $this->mdl->isUpdate(true)->allowField((array)$this->local['whiteList'])->save();
        ...
        $this->setTitle("修改{$this->mdl->cname}[ID:{$id}]", 'operation');
    ...
    public function delete()
    {
        ...
        $pk = $this->mdl->getPk();
        ##获取到需要删除的数据
        $delete_model = $this->mdl->where(array_merge((array)$this->local['where'], array($pk => array('eq', $id))))->find();
    ...
    public function batch_delete()
    {
        if ($this->request->isPost()) {
            ...
            ##获取到需要删除的数据
            $delete_model = $this->mdl->where(array_merge((array)$this->local['where'], array($pk => array('in', $selected_ids))))->select();
    ...
    public function sort()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->args['parent_id'] && $this->local['parent_conj']) {
                $this->local['where'][$this->local['parent_conj']] = $this->args['parent_id'];
            }
            ...
            $this->setTitle("{$this->mdl->cname}排序", 'operation');
    ...
    public function lists()
    {
        ...
        if ($use_picture_model) {
            $use_picture_model = json_decode($use_picture_model);
        } else {
            $use_picture_model = [];
        }
        ...
        if (isset($this->args['parent_id']) && isset($this->local['parent_conj']) && !isset($this->local['where'][$this->local['parent_conj']])) {
            $this->local['where'][$this->local['parent_conj']] = $this->args['parent_id'];
        }
        ...
        if (setting('is_admin_cache')) {
            $listCache = \think\Cache::get($requestUrl);
            if ($listCache) {
                $isQuery = false;
        ...
        if ($isQuery) {
            ...
            if (setting('is_admin_cache')) {
                \think\Cache::tag($this->m)->set($requestUrl, ['list' => $this->list, 'page' => $this->page], 3600);
            }
        }
        /*
        $this->list($this->mdl);        
        ...
        */
        ...
        if (isset($this->mdl->parentModel) && $this->mdl->parentModel != 'parent') {
            ...
            $this->addAction("返回父级", $this->local['parent_return_url'], 'fa-reply');
        }
    ...
    public function parseListFilters()
    {
        ...
        foreach ($fields as $field => &$info) {
            ...
            if (!isset($info['elem'])) {
                ...
                } elseif ($this->mdl->form[$field]['elem'] === 'date') {
                    $info['elem'] = 'date';
                } elseif ($this->mdl->form[$field]['elem'] === 'datetime') {
                    $info['elem'] = 'datetime';
                } else {
                    $info['elem'] = 'text';
                }
            ...
            if (array_key_exists($field, (array)$this->mdl->form) && !isset($where_result)) {
                if (in_array($info['type'], ['text', 'string'])) {
                    $this->local['where'][$field] = ['LIKE', '%' . $val . '%'];
