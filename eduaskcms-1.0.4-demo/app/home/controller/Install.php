...
class Install extends Home
{
    ...
    public function index()
    {
        ...
        echo '测试版本用于学习，安装数据请自行先建好库；修改database配置；导入根目录database.sql文件即可';
        exit;
        ...
        if (!file_exists(dirname(APP_PATH) . DS . 'database.sql')) {
            echo '失败：' . dirname(APP_PATH) . DS . 'database.sql' . '文件不存在';
            exit ;
        }
        
        $dbSql = file_get_contents(dirname(APP_PATH) . DS . 'database.sql');
        $dbSql = str_replace("\r", "\n", $dbSql);
        $dbSql = explode(";\n", $dbSql);
        //替换表前缀
        $default_tablepre = "eduask_";
        ...
        if (!$count) {
            $insert  = "INSERT INTO `eduask_admin_menu` (`id`, `parent_id`, `title`, `url`, `icon`, `is_nav`, `is_debug`, `list_order`) VALUES
(1, 0, '后台导航', '', '', 0, 0, 0),
(2, 1, 'CMS', '', '', 1, 0, 0),
(3, 1, '微信', '', '', 1, 0, 0),
(4, 1, '商城', '', '', 0, 0, 0),
...
(32, 30, '权限节点', '/run/power_tree/lists', 'fa-tree', 1, 0, 0);";  
        ...
        if (!$count) {
            ...
            $this->User->save(['id' => 1, 'username' => 'eduask', 'password' => 'eduask', 'user_group_id' => $group[0]['id'], 'status' => 'verified']);
            echo '成功：用户安装成功<br>';
        } else {
            echo '失败：用户已安装<br>';
        }
        ...
        if (!$count) {
            ...
            $insert = "INSERT INTO `eduask_model` (`id`, `model`, `cname`, `is_menu`, `is_power`, `is_dustbin`, `created`, `modified`) VALUES
...
(26, 'QueryData', '数据查询', 0, 0, 0, '{$now}', '{$now}'),
(27, 'Exlink', '栏目外链', 1, 0, 0, '{$now}', '{$now}'),
(28, 'PowerTree', '权限节点', 0, 0, 0, '{$now}', '{$now}'),
(29, 'Power', '权限', 0, 0, 0, '{$now}', '{$now}');";
            $insert = str_replace(" `{$default_tablepre}", " `{$table_prefix}", $insert); 
        ...
        if (!$count) {
            $insert = "INSERT INTO `eduask_query_data` (`id`, `title`, `query`, `controller`, `menu_id`, `list_count`, `is_family`, `is_verify`, `type`, `contain`, `where`, `field`, `order`) VALUES
(1, '示范数据', 'index', '', 2, 4, 1, 0, 'select', '[''Menu'']', '[''is_index''=>1]', '[''id'',''title'',''date'',''image'']', '[''is_index''=>''DESC'']');";
        ...
        echo '<a href="' . $absroot . '" style="color:green;">访问前台</a>　　<a href="' . $absroot . 'run" style="color:green;">访问后台</a>　注：默认用户名和密码：eduask<br>';        
