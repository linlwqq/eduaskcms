...
class callback
{
    ...
    {
        ##前台每个页面都需要家长的css 和js 
        $this->ts->assign->addJs('jquery-1.11.1.min');
        $this->ts->assign->addJs('common');
    ...
    public function appBeforeRun()
    {
        ...
        $this->ts->assign->addJs('/files/layui-v2.1.7/layui.js');
        $this->ts->assign->addJs('admin/global.js');
        $this->ts->assign->addCss('/files/artDialog/css/simple.css');

        $this->ts->assign->addCss('/files/layui-v2.1.7/css/layui.css');
