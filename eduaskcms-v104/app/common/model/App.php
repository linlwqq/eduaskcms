...
class App extends Model
{
    ...
    */
    ...
    public function initialize()
    {
        ...
        if (!isset($this->form['created'])) {
            $this->createTime = false;##关闭字段
        }
        if (!isset($this->form['modified'])) {
            $this->updateTime = false;##关闭字段
        }
    }
    ...
    {
        $this->old_data = array();##清空原数据
        if ($this->isUpdate) {
            ##更新  
            $scene = 'edit';##验证场景

            if ($this->data['id']) {
                $old_data = $this->where(array('id' => $this->data['id']))->find();
                if ($old_data) {
    ...
    protected function add_dustbin()
    {
        if ($this->data) {
            ...
            $data['title'] = $this->data[$this->display];
            $data['data'] = gzcompress(serialize($this->data));
            $data['status'] = 0;
        ...
        if ($this->assoc && $this->data['id']) {
            foreach ($this->assoc as $assocModel => $assocInfo) {
                if (in_array($assocInfo['type'], ['hasOne', 'hasMany']) && $assocInfo['deleteWith'] === true) {
                    ...
                    model($assocModel)->where([$foreignKey => $this->data['id']])->delete();
                }
            }
        }
    }
    ...
    public function validateCheck($data = null, $rule = null, $batch = null, $scene = null)
    {
        ...
        if (empty($this->validate_rule)) {
            $this->validateFormat($this->_validate);
        }
        if (empty($rule)) {
        ...
        if ($scene) {
            $this->validate_object->scene($scene, $this->validate_scene[$scene]);
        }
        ...
        if (!$this->validate_object->batch()->scene($scene)->check($data)) {
            $this->error = array_merge($this->validate_object->getError(), (array)$this->error);
        }
        ##有误
        if (!empty($this->error)) {
            return false;
    ...
    function processUpload()
    {
        ...
        if ($_uploads_info) { 
            foreach ($_uploads_info as $field => $info) {
                ...
                if (!$file) {
                    $this->forceError($field, $GLOBALS['bbit_upload_file_error']);
                    continue;
                }
                ...
                if ($elem === 'image') {
                    if (isset($this->form[$field]['image']['thumb'])) { 
                        ...
                        list($thumb_width, $thumb_height, $thumb_method) = $this->getThumbInfo([$thumb_width, $thumb_height, $thumb_method], $this, $this->data);
                        ...
                        if ($rslt) {
                            ...
                            $this->data[$this->form[$field]['image']['thumb']['field']] = $thumb;
                        }
                    }
                    ...
                    if (isset($this->form[$field]['image']['resize'])) {
                        ...
                        list($resize_width, $resize_height, $resize_method) = $this->getResizeInfo([$resize_width, $resize_height, $resize_method], $this, $this->data);
                ...
                if ($this->form[$field]['upload']['nameField']) {
                    $this->data[$this->form[$field]['upload']['nameField']] = $info['name'];
                }
                if ($this->form[$field]['upload']['sizeField']) {
                    $this->data[$this->form[$field]['upload']['sizeField']] = $info['size'];
                }                
    ...
    public function getArray($data = null, $assocList = true)
    {
        ...
        if (is_array($data)) {
            foreach ($data as $key => $value) {
                $data[$key] = $this->getArray($value, $assocList);
            }
        }
        return $data;
    }
