...
class Form
{
    ...
    {
        $this->requestObj = \think\Request::instance();
    ...
    public function create($options = array('type' => 'post', 'mdl' => null, 'url' => null, 'inputDefaults' => array('label' => false, 'token' => true)))
    {
        ...
        if ($this->inputDefaults['token']) {
            ...
            if ($token['code'] && $token['created'] >= time() - 3600) {
                $code = $token['code'];
            } else {
    ...
    public function radio($name, $options = array(), $infos = array())
    {
        $info = $this->getInfo($name, $infos);
        if ($info['nameStr']) {
            $eval = "\$radioval=\$this->" . $info['nameStr'] . ';';
            eval($eval);
    ...
    public function checkbox($name, $options = array(), $infos = array())
    {
        $info = $this->getInfo($name, $infos);
        if ($info['nameStr']) {
            $eval = "\$checkboxval=\$this->" . $info['nameStr'] . ';';
            eval($eval);
            $checkboxval = $info['value'] ? $info['value'] : $checkboxval;
            if (!is_array($checkboxval) && static::isJson($checkboxval)) $checkboxval = json_decode($checkboxval, true);
        }
        $optStr = '';
        if (!empty($options)) {
